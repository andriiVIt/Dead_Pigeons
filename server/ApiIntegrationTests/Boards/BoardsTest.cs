...
public async Task CreateBoard_ReturnsOkAndBoard()
{
    ...
    [Fact]
    public async Task DeleteBoard_RemovesBoardFromDatabase()
    {
        // Arrange
        using (var scope = ApplicationServices.CreateScope())
        {
            ...
            // Act
            var response = await Client.DeleteAsync($"/api/board/{board.Id}");

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.NoContent);

            // Перевірка, що дошка видалена
            var deletedBoard = ctx.Boards.Find(board.Id);
            deletedBoard.Should().BeNull();

            // Баланс гравця має залишитися незмінним (в цьому випадку)
            var updatedPlayer = ctx.Players.Find(player.Id);
            updatedPlayer.Should().NotBeNull();
            updatedPlayer!.Balance.Should().Be(100m); // Баланс не змінюється при видаленні дошки
        }
    }
}
